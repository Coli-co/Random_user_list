...
function showUserInfo(id) {
  // name, gender, age , region, birthday, email
...
function removeUser(id) {
  // 避免收藏清單是空的，或傳入的 id 在收藏清單中不存在，就結束這個函式
  if (!userList || !userList.length) return

  // 尋找 user 名單的 index
  const userIndex = userList.findIndex((user) => user.id === id)
  if (userIndex === -1) return //index 不存在於名單中，結束函式

  userList.splice(userIndex, 1) // 移除 user 名單
  // 更新收藏清單資料
